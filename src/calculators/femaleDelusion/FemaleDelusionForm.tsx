...
interface FormValues {
  ...
  [key: string]: number; // Add index signature to fix type error
}

interface FemaleDelusionFormProps {
  onCalculate: (results: { score: number, answers: Record<string, number> }) => void;
  onReset: () => void;
}

const FemaleDelusionForm: React.FC<FemaleDelusionFormProps> = ({ onCalculate, onReset }) => {
  const [isCalculating, setIsCalculating] = useState(false);

  const form = useForm<FormValues>({
    defaultValues: {
      financialExpectations: 5,
  ...
  const onSubmit = (data: FormValues) => {
    ...
    const weightedScores = {
      financialExpectations: combinedAnswers.financialExpectations * 1.5,
      appearanceStandards: combinedAnswers.appearanceStandards * 0.8,
      careerStatus: combinedAnswers.careerStatus * 1.2,
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem className="space-y-4">
              <div>
                <FormLabel className="text-base">Financial Expectations</FormLabel>
                <FormDescription>
                  How important is a potential partner's financial status and income level to you?
                </FormDescription>
              </div>
              <FormControl>
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem className="space-y-4">
              <div>
                <FormLabel className="text-base">Appearance Standards</FormLabel>
                <FormDescription>
                  How rigid are your standards for a partner's physical appearance (height, fitness, etc.)?
                </FormDescription>
              </div>
              <FormControl>
                <div className="space-y-2">
          ...
          render={({ field }) => (
            <FormItem className="space-y-4">
              <div>
                ...
                <FormDescription>
                  How important is your potential partner's career prestige or professional status?
                </FormDescription>
              </div>
              <FormControl>
                <div className="space-y-2">
          ...
          render={({ field }) => (
            <FormItem className="space-y-4">
              <div>
                ...
                <FormDescription>
                  How strict are you about a partner's age being older than you or within a specific range?
                </FormDescription>
              </div>
              <FormControl>
                <div className="space-y-2">
...
export default FemaleDelusionForm;
